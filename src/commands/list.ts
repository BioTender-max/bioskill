import { loadCatalog, Skill } from "../catalog.js";
...
export async function listCommand(opts: ListOptions): Promise<void> {
  ...
  for (const s of filtered) {
    if (opts.long) {
      ...
      if (s.description) log.raw(`  ${truncate(s.description, 280)}`);
      log.raw("");
    } else {
...
export function filter(skills: Skill[], opts: { category?: string; source?: string }): Skill[] {
  return skills.filter((s) => {
    if (opts.category && s.category.toLowerCase() !== opts.category.toLowerCase()) return false;
    if (opts.source && s.source.toLowerCase() !== opts.source.toLowerCase()) return false;
    return true;
  });
}
...
function sortByCount<T>(m: Map<string, T[]>): [string, T[]][] {
